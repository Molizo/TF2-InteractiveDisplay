-- JSON Code by https://github.com/Egor-Skriptunoff/json4lua/blob/master/json.lua
-- Actual custom code starts at the end
...
-- Here is where the mod code starts

local lastRanCounter = 0 -- Used to prevent the mod from running on every engine update
local lastJSONFileCounter = 0 -- Used to keep count of the index for the JSON files so there's not a file access conflict by using multiple files
...
function data()
	return {
		...
		update = function()
            if lastRanCounter == 30 then
                local lineIDs = game.interface.getLines()
                local lines = {}
                ...
                local vehicles = {}
                for i, vehicleID in ipairs(vehicleIDs) do
                    local vehicle = game.interface.getEntity(vehicleID)
                    vehicles[i] = {
                        id = vehicle.id,
                        depot = vehicle.depot,
                        line = vehicle.line,
                ...
                local stationIDs = game.interface.getStations()
                local stations = {}
                ...
                if lastJSONFileCounter > 9 then
                    lastJSONFileCounter = 0
                end
            else
                lastRanCounter = lastRanCounter + 1
            end
		end,
		guiUpdate = function()
		end,
